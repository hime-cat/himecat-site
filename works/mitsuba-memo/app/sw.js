const CACHE_NAME = "mitsuba-memo-v29";

const APP_SHELL = [
  "./",
  "./index.html",
  "./css/style.css",
  "./js/script.js",
  "./manifest.webmanifest",
  "./images/fabicon.png",
  "./images/apple-touch-icon-kuma.png",
  "./images/icon-192-kuma.png",
  "./images/icon-512-kuma.png",
  "./images/mitsuba-memo-icon.png",
  "./images/kuma-tab.png",
  "./images/kuma-hand.png",
  "./images/hokapan-tab.png",
  "./images/hokapan-smile-tab.png",
  "./images/input-section-bg.webp",
  "./images/decor/clover.webp",
  "./images/decor/leaf.webp",
  "./images/ogp.png"
];

const INDEX_URL = new URL("./index.html", self.registration.scope).href;
const SHELL_URLS = new Set(APP_SHELL.map(path => new URL(path, self.registration.scope).href));

self.addEventListener("install", event => {
  event.waitUntil(
    caches.open(CACHE_NAME)
      .then(cache => cache.addAll(APP_SHELL))
      .then(() => self.skipWaiting())
  );
});

self.addEventListener("activate", event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          .filter(key => key !== CACHE_NAME)
          .map(key => caches.delete(key))
      ))
      .then(() => self.clients.claim())
  );
});

self.addEventListener("fetch", event => {
  const { request } = event;

  if (request.method !== "GET") return;

  const url = new URL(request.url);

  if (url.origin !== location.origin) return;

  if (request.mode === "navigate") {
    event.respondWith(
      fetch(request)
        .then(response => {
          const copy = response.clone();
          caches.open(CACHE_NAME).then(cache => cache.put(INDEX_URL, copy));
          return response;
        })
        .catch(() => caches.match(INDEX_URL))
    );
    return;
  }

  if (!SHELL_URLS.has(url.href)) return;

  event.respondWith(
    caches.match(request)
      .then(cached => cached || fetch(request).then(response => {
        const copy = response.clone();
        caches.open(CACHE_NAME).then(cache => cache.put(request, copy));
        return response;
      }))
  );
});
